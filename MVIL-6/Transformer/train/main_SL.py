...
def get_Focal_loss(logits, label, FocalLoss):
    loss = FocalLoss.forward(logits, label)
...
def periodic_test(test_iter, model, criterion, config, sum_epoch):
    ...
    print('T5_BERT_Model current performance')
...
def train_ACP(train_iter, valid_iter, test_iter, model, optimizer, criterion, config, iter_k):
    ...
    for epoch in range(1, config.epoch + 1):
        ...
        for batch in train_iter:
            ...
            # loss = get_val_loss(logits, label, criterion)

            # loss = get_Focal_loss(logits, label, criterion)
...
def train_test(train_iter, test_iter, config):
    print('=' * 50, 'train-T5_BERT_Model', '=' * 50)
    print('len(train_iter)', len(train_iter))
    print('len(test_iter)', len(test_iter))
...
def select_dataset():
    ...
    path_test_data = '../data/Validate.tsv'
